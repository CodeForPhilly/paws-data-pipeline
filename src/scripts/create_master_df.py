...
def main(conn):
    ...
    master_df = petpoint[['outcome_person_id', 'out_email']].rename(columns={'outcome_person_id': 'petpoint_id', 'out_email': 'email_address'})

    # Merge Salesforce data to the master_df
    salesforcecontacts = pd.read_sql('select * from salesforcecontacts', conn)
    master_df = master_df.merge(salesforcecontacts[['account_id', 'email_address']].rename(
        columns={'account_id': 'salesforce_id'}), how='outer')
    ...
    master_df = master_df.merge(volgistics[['number', 'email']].rename(columns={'number' : 'volgistics_id', 'email': 'email_address'}), how='outer')
