...
master_df = create_user_master_df(conn, 'master_df',
                                  '(master_id INT PRIMARY KEY NOT NULL, petpoint_id text, volgistics_id text, salesforce_id text )')

# TODO: Match accounts based on email addresses

# create a dataframe with links to all other tables
master_with_petpoint = (
    master_df.drop(['petpoint_id'], axis=1).merge(
        petpoint[['outcome_person_']].rename(columns={'outcome_person_': 'petpoint_id'}), how='left').assign(
        petpoint_name=lambda df: df['petpoint_id'].map(clean_entry))
)
...
master_with_petpoint_and_volgistics_and_salesforce = (
    master_df.drop(
        ['salesforce_id'].merge(salesforcecontacts[['account_id']].rename(columns={'account_id': 'salesforce_id'}),
                                how='left').assign(salesforce_name=lambda df: df['salesforce_id'].map(clean_entry))
    )
# master_with_petpoint.head()
