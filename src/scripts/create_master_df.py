...
def __find_and_add_new_rows(connection, new_rows_dataframe):
    if new_rows_dataframe is None:
        return

    df_already_exists = engine.dialect.has_table(engine, 'master_df')
    ...
    if not df_already_exists:
        ...
        new_rows_dataframe.to_sql('master_df', connection, index_label='master_id')
    # if the master_df table exists, append these rows to it
        ...
        master_df.to_sql('master_df', connection, index_label='master_id', if_exists='replace') # TODO - master_id is getting duplicated. Fix this.
...
def start(rows_for_master_df):
    ...
    current_app.logger.info('testing create_master_df.start()')
    ...
    current_app.logger.info('running create_master_df.start()')
    new_rows_dataframe = None
    if "new_matches" in rows_for_master_df:
        new_rows_dataframe = pd.DataFrame.from_dict(rows_for_master_df["new_matches"])
    updated_rows_dataframe = None
    if "updated_rows" in rows_for_master_df:
        updated_rows_dataframe = pd.DataFrame.from_dict(rows_for_master_df["updated_rows"])
    ...
    with engine.connect() as connection:
        ...
        current_app.logger.info('   - finish create_master_df.start()')
